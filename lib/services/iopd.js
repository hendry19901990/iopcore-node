...
var Proposal = iopcore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to iopd in Node.js. Manages starting and
 * stopping iopd as a child process for application support, as well as connecting
 * to multiple iopd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Iop(options) {
  if (!(this instanceof Iop)) {
    return new Iop(options);
  }
...
util.inherits(Iop, Service);

Iop.dependencies = [];

Iop.DEFAULT_MAX_TXIDS = 1000;
Iop.DEFAULT_MAX_HISTORY = 50;
Iop.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Iop.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Iop.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Iop.DEFAULT_SPAWN_RESTART_TIME = 5000;
Iop.DEFAULT_SPAWN_STOP_TIME = 10000;
Iop.DEFAULT_TRY_ALL_INTERVAL = 1000;
Iop.DEFAULT_REINDEX_INTERVAL = 10000;
Iop.DEFAULT_START_RETRY_INTERVAL = 5000;
Iop.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Iop.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Iop.DEFAULT_INSTANTSEND_FEE = 100000;
Iop.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Iop.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Iop.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Iop.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Iop.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Iop.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Iop.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Iop.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Iop.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Iop.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Iop.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Iop.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Iop.prototype._initCaches = function() {
  ...
  this.addressImported = LRU(100000);

  // caches valid indefinitely
  this.transactionCache = LRU(100000);
  this.transactionByHashCache = LRU(100000);
...
Iop.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Iop.prototype.getAPIMethods = function() {
  var methods = [
    ['getBlock', this, this.getBlock, 1],
    ['getRawBlock', this, this.getRawBlock, 1],
    ['getBalance', this, this.getBalance, 1],
    ...
    ['getNetworkHashps', this, this.getNetworkHashps, 0],
    ['getSupply', this, this.getSupply, 0],
    ['syncPercentage', this, this.syncPercentage, 0],
    ['isSynced', this, this.isSynced, 0],
    ['importAddress', this, this.importAddress, 1],
    ['getRawTransaction', this, this.getRawTransaction, 1],
    ['getListTransactions', this, this.getListTransactions, 1],
    ['getListTransactionsByAddress', this, this.getListTransactionsByAddress, 1],
    ['getTransaction', this, this.getTransaction, 1],
    ['getTransactionByHash', this, this.getTransactionByHash, 1],
    ...
    ['getAddressSummaryDB', this, this.getAddressSummaryDB, 1],
    ['generateBlock', this, this.generateBlock, 1],
    ['getRichList', this, this.getRichList, 1]
  ];
  return methods;
};
 ...
 */
Iop.prototype.getPublishEvents = function() {
  return [
    {
...
Iop.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'Iopd/' + name, 'total:', this.subscriptions[name].length);
};

Iop.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'Iopd/' + name, 'total:', this.subscriptions[name].length);
};

Iop.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  for(var i = 0; i < addresses.length; i++) {
    if (iopcore.Address.isValid(addresses[i], this.node.network)) {
      addAddress(addresses[i]);
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'Iopd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Iop.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'Iopd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Iop.prototype.unsubscribeAddressAll = function(emitter) {
...
Iop.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Iop.DEFAULT_CONFIG_SETTINGS;
...
Iop.prototype._parseIopConf = function(configPath) {
  var options = {};
...
Iop.prototype._expandRelativeDatadir = function() {
...
Iop.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in iopd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in iopd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in iopd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './iop.conf');

  log.info('Using Iop config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseIopConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseIopConf(networkConfigPath));
  }
...
Iop.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of iopcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of iopcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of iopcore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in iop.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that iopd is undergoing a reindex. ' +
             ...
             'of iopcore-node services will start.');
    node._reindex = true;
  }
};

Iop.prototype._resetCaches = function() {
  this.transactionDetailedCache.reset();
  this.transactionByHashCache.reset();
  this.utxosCache.reset();
  this.txidsCache.reset();
  this.balanceCache.reset();
  this.summaryCache.reset();
  this.blockOverviewCache.reset();
  this.addressImported.reset();
};

Iop.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Iop.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Iop.prototype._initChain = function(callback) {
  var self = this;
...
Iop.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === iopcore.Networks.testnet) {
    networkOptions.rpcport = 9998;
  }
  return networkOptions;
};

Iop.prototype._getNetworkConfigPath = function() {
  var networkPath;
  /*if (this.node.network === iopcore.Networks.testnet) {
    networkPath = 'testnet3/iop.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/iop.conf';
    }
  }*/
  return networkPath;
};

Iop.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === iopcore.Networks.testnet) {
...
Iop.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('Iopd/hashblock', message.toString('hex'));
    }
  }

};

Iop.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Iop.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('IoP Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Iop.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Iop.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('Iopd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Iop.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = iopcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('Iopd/rawtransaction', message.toString('hex'));
    }

    var tx = iopcore.Transaction();
    tx.fromString(message);
    var txid = iopcore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Iop.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = iopcore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('Iopd/transactionlock', message.toString('hex'));
    }

  }
};

Iop.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Iop.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Iop.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Iop.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Iop.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('IoP Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Iop.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Iop.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Iop.prototype._stopSpawnedIop = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/iopd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned IoP process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean IoP process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Iop.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedIop(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting Iop process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('IoP process unexpectedly exited with code:', code);
        log.warn('Restarting Iop child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('IoP process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn iopd.'));
      }
...
Iop.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to iopd.'));
    }
 ...
 */
Iop.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Iop configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Iop.prototype.isSynced = function(callback) {
...
/**
 * ImportAddress just to request balance
 * @param {Function} callback
 */
Iop.prototype.importAddress = function(address, callback) {
    var self = this;
	var cacheKey = address;
	var readyImp = self.addressImported.get(cacheKey);
	
	if (readyImp){
		return setImmediate(function() {
           callback(null, readyImp);
        });
	}else{
		self._tryAllClients(function(client, done) {
		  client.importAddress(address, address, true, function(err, response) {
            if (err) {
               return done(self._wrapRPCError(err));
            }
			self.addressImported.set(cacheKey, response.result);
 ...
 * @param {Function} callback
 */
Iop.prototype.syncPercentage = function(callback) {
  var self = this;
...
Iop.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, iopcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Iop.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Iop.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = iopcore.Script.fromAddress(delta.address);
    return {
...
Iop.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Iop.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Iop.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in iopcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Iop.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 16 */
  var self = this;
  var txids = [];
  /*self.getListTransactionsByAddress(addressArg, function(er, re){
       
     if(er)
        return callback(er);

     re.forEach(function(tx, i){
          txids.push(tx.txid);
     });
  ...
  self._explorer.find_address(addressArg, function(res){
	  
	  if (!res){
		  callback(null, txids);
	  }else{
		  
		 if (res === undefined || res === null || !res.txs)
			 callback(null, txids); 
		  
		 res.txs.forEach(function(tx, i){
			  txids.push(tx.addresses);
		 });
...
Iop.prototype._getConfirmationsDetail = function(transaction) {
...
Iop.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Iop.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Iop.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: [],
    satoshis: 0
  };
 ...
 * @param {Object} txid - A IoP transaction id
 * @param {Function} callback
 */
Iop.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
  ...
  self.getDetailedTransaction(
    ...
    function(err, transaction) {
      ...
      var details = {
        ...
        confirmations: (transaction.confirmations > 0) ? 1 : 0,
        tx: transaction
      };
      next(null, details);
    }
...
Iop.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof iopcore.Address) {
...
Iop.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Iop.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
Iop.prototype.getAddressSummaryDB = function(addressArg, callback) {
    this._explorer.find_address(addressArg, function(result){
		callback(result);
	}); 
};
 /**
 ...
 * @param {Function} callback
 */
Iop.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Iop.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
  
  if (!blockArg) callback(null, blockArg);
 ...
 * @param {Function} callback
 */
Iop.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Iop.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
  
  if (!blockArg) return callback("blockhash is undefined");
...
/**
 * Will retrieve a block as a iopcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Iop.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = iopcore.Block.fromString(response.result);
...
/**
 * Will retrieve a getBalance 
 * @param {String} Account Name
 * @param {Function} callback
 */
Iop.prototype.getBalance = function(address, callback) {
	var self = this;
    self._tryAllClients(function(client, done) {
		client.listTransactions(address, 1500,0, true, function(err, response) {
          if (err) {
            return done(self._wrapRPCError(err));
          }
		  var balance = 0;
		  var arr_ = response.result;
		  arr_.forEach(function(tx, i){
			 balance += tx.amount;
		  });
          done(null, balance);
        });
...
/**
 * Will retrieve the transactions of address 
 * @param {String} address
 * @param {Function} callback
 */
Iop.prototype.getListTransactionsByAddress = function(address, callback) {
	var self = this;
    self._tryAllClients(function(client, done) {
		client.listTransactions(address, 1500,0, true, function(err, response) {
          if (err) {
 ...
 * @param {Function} callback
 */
Iop.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Iop.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Iop.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Iop.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Iop.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a iopcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Iop.prototype.getTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a iopcore Transaction from a hashid. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Iop.prototype.getTransactionByHash = function(txid, callback) {
  var self = this;
  var tx = self.transactionByHashCache.get(txid);
  if (tx) {
    return setImmediate(function() {
      callback(null, tx);
    });
    ...
    self._tryAllClients(function(client, done) {
      client.getTransaction(txid, function(err, response) {
        ...
        self.transactionByHashCache.set(txid, response.result);
        done(null, response.result);
      });
    }, callback);
  }
...
/**
 * Will get transaction by Address as a iopcore Transaction. Results include the mempool.
 * @param {Function} callback
 */
Iop.prototype.getListTransactions = function(address, callback) {
	
   	var self = this;
	self._tryAllClients(function(client, done) {
	   	client.listTransactions(address, 1000,0, true,function(err, response) {
			done(null, response.result);
		});	
	}, callback);
 ...
 * @param {Function} callback
 */
Iop.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addInputsToTx(input, CB) {
     ...
     if(input.hasOwnProperty('coinbase') || !input.hasOwnProperty('txid')){
          tx.inputs.push({
			prevTxId: input.txid | null,
			outputIndex: input.vout | null ,
			script: script,
     ...
     CB(null, true); // input is NULL

     /*self._tryAllClients(function(client, done) {
	  client.getRawTransaction(input.txid, 1, function(err, response) {
                if(err) 
                    done(null, true);
                 var addressIn = 0;
                 var value_ = 0;
                 if(!err && 
				       response.result.vout[0].scriptPubKey.hasOwnProperty('addresses')){ 
					   addressIn = response.result.vout[0].scriptPubKey.addresses[0];
					}
		         if(!err && addressIn != 0 && response.result.vout[0].scriptPubKey.hasOwnProperty('value')) value_ = response.result.vout[0].value;
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (fee_tx >= Iop.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.001 check for txlock
      var rawTx = new iopcore.Transaction(result.hex);
      var hash = iopcore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.Iopd.zmqKnownTransactionLocks.get(id)) {
  ...
  if (tx) {
   ...
   // input is NULL
   /* self.getTransactionByHash(txid, function(err, response) {
       if (!err)
          fee_tx = response.fee;
    });*/
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        self.getBlockOverview(result.blockhash, function(err_, res_) {      
                if (!err_)
                  height_tx = res_.height;
		...
		addTxlockToTx(tx, result);
                ...
                async.map(result.vin, addInputsToTx, function(er, suc){
 ...
 * @param callback
 */
Iop.prototype.govObjectList = function(options, callback) {
  var self = this;
 ...
 * @param callback
 */
Iop.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Iop.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Iop.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Iop.prototype.getInfo = function(callback) {
  var self = this;
...
Iop.prototype.getSupply = function(callback) {
  this._explorer.get_supply(function(count) {  
     return callback(count);
  });
};

Iop.prototype.getNetworkHashps = function(callback) {
   var self = this;
   this.client.getNetworkHashps(function(err, response) {
       if (err) {
		  return callback(self._wrapRPCError(err));
		}
		return callback(null, ((response.result / 1000000).toFixed(4)));
   });
};

Iop.prototype.generateBlock = function(num, callback) {
  var self = this;
...
Iop.prototype.getRichList = function(pageNumber, callback) {
  var self = this;
  var pageNumber = (!pageNumber)? 1 : pageNumber;
  self._explorer.richt_list_count(function(count_) {
	var resultRichList = {
		count: 0,
		totalsupply: 0,
		list: []
    ...
    if (count_) {
		resultRichList.count = count_.count;
		resultRichList.totalsupply = count_.totalsupply;
		self._explorer.richt_list(pageNumber, function(list) {
			
			if(list != 0)
				resultRichList.list = list;
			
			callback(resultRichList);
		});
	}else{
		callback(resultRichList);
	}
 ...
 * @param {Function} callback
 */
Iop.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('iopd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('iopd process did not exit'));
      }
