...
/**
 * Will return the path and default iopcore-node configuration. It will search for the
 * configuration file in the "~/.iopcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.iopcore');
  var defaultConfigFile = path.resolve(defaultPath, './iopcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['iopd', 'web'];
      ...
      servicesConfig: {
        iopd: {
          spawn: {
            ...
            exec: path.resolve(__dirname, defaultPath, './data/iopd')
          }
        }
      }
    };
