...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for iopd is necessary for this upgrade with the "reindex=1" IoP.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and iopcore-node.json. \n\n' 
                 );
        ...
        servicesConfig: {
          iopd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/iopd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed iopcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './iopcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in iopcore-node services directory
    ...
    if (servicePackage.iopcoreNode) {
      serviceModule = service.name + '/' + servicePackage.iopcoreNode;
    }
 ...
 * @param {Node} node - The IoP Node instance
 */
