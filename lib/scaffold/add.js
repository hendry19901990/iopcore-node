...
function add(options, done) {
  ...
  var iopcoreConfigPath = path.resolve(configPath, 'iopcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(iopcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      addService(configPath, service, function(err) {
        ...
        // add service to iopcore-node-dash.json
        addConfig(iopcoreConfigPath, serviceName, next);
      });
    }, done
  );
}
