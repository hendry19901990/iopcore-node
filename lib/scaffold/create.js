...
var BASE_PACKAGE = {
  description: 'A full IoP node build with IoPcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'iopcore-lib': '^' + iopcore.version,
    'iopcore-node': version
  }
};
...
/**
 * Will create a base iopcore Node configuration directory and files.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/iopcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a iopcore Node directory, configuration file,
 * iop configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the iopcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the iopcore-node directory and configuration
