...
describe('IoP Service', function() {
  ...
  var baseConfig = {
    node: {
      network: iopcore.Networks.testnet
    },
    spawn: {
      datadir: 'testdir',
      exec: 'testpath'
  ...
  describe('@constructor', function() {
    it('will create an instance', function() {
      var iopd = new BitcoinService(baseConfig);
      should.exist(iopd);
    });
    it('will create an instance without `new`', function() {
      var iopd = BitcoinService(baseConfig);
      should.exist(iopd);
    });
    it('will init caches', function() {
      var iopd = new BitcoinService(baseConfig);
      should.exist(iopd.utxosCache);
      should.exist(iopd.txidsCache);
      should.exist(iopd.balanceCache);
      should.exist(iopd.summaryCache);
      should.exist(iopd.transactionDetailedCache);

      should.exist(iopd.transactionCache);
      should.exist(iopd.rawTransactionCache);
      should.exist(iopd.blockCache);
      should.exist(iopd.rawBlockCache);
      should.exist(iopd.blockHeaderCache);
      should.exist(iopd.zmqKnownTransactions);
      should.exist(iopd.zmqKnownBlocks);
      should.exist(iopd.lastTip);
      should.exist(iopd.lastTipTimeout);
    });
    it('will init clients', function() {
      var iopd = new BitcoinService(baseConfig);
      iopd.nodes.should.deep.equal([]);
      iopd.nodesIndex.should.equal(0);
      iopd.nodes.push({client: sinon.stub()});
      should.exist(iopd.client);
    });
    it('will set subscriptions', function() {
      var iopd = new BitcoinService(baseConfig);
      iopd.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: [],
        transactionlock: []
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var iopd = new BitcoinService(baseConfig);
      iopd._initDefaults({transactionConcurrency: 10});
      iopd.transactionConcurrency.should.equal(10);
      iopd._initDefaults({});
      iopd.transactionConcurrency.should.equal(5);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var iopd = new BitcoinService(baseConfig);
      var methods = iopd.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var iopd = new BitcoinService(baseConfig);
      var events = iopd.getPublishEvents();
      should.exist(events);
      events.length.should.equal(4);
      events[0].name.should.equal('iopd/rawtransaction');
      events[0].scope.should.equal(iopd);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('iopd/transactionlock');
      events[1].scope.should.equal(iopd);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('iopd/hashblock');
      events[2].scope.should.equal(iopd);
      events[2].subscribe.should.be.a('function');
      events[2].unsubscribe.should.be.a('function');
      events[3].name.should.equal('iopd/addresstxid');
      events[3].scope.should.equal(iopd);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var iopd = new BitcoinService(baseConfig);
      iopd.subscribe = sinon.stub();
      iopd.unsubscribe = sinon.stub();
      var events = iopd.getPublishEvents();

      events[0].subscribe('test');
      iopd.subscribe.args[0][0].should.equal('rawtransaction');
      iopd.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      iopd.unsubscribe.args[0][0].should.equal('rawtransaction');
      iopd.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      iopd.subscribe.args[1][0].should.equal('transactionlock');
      iopd.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      iopd.unsubscribe.args[1][0].should.equal('transactionlock');
      iopd.unsubscribe.args[1][1].should.equal('test');

      events[2].subscribe('test');
      iopd.subscribe.args[2][0].should.equal('hashblock');
      iopd.subscribe.args[2][1].should.equal('test');

      events[2].unsubscribe('test');
      iopd.unsubscribe.args[2][0].should.equal('hashblock');
      iopd.unsubscribe.args[2][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var iopd = new BitcoinService(baseConfig);
      var emitter = {};
      iopd.subscribe('hashblock', emitter);
      iopd.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      iopd.subscribe('rawtransaction', emitter2);
      iopd.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      iopd.subscribe('hashblock', emitter1);
      iopd.subscribe('hashblock', emitter2);
      iopd.subscribe('hashblock', emitter3);
      iopd.subscribe('hashblock', emitter4);
      iopd.subscribe('hashblock', emitter5);
      iopd.subscriptions.hashblock.length.should.equal(5);

      iopd.unsubscribe('hashblock', emitter3);
      iopd.subscriptions.hashblock.length.should.equal(4);
      iopd.subscriptions.hashblock[0].should.equal(emitter1);
      iopd.subscriptions.hashblock[1].should.equal(emitter2);
      iopd.subscriptions.hashblock[2].should.equal(emitter4);
      iopd.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var iopd = new BitcoinService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      iopd.subscriptions.hashblock= [emitter1];
      iopd.unsubscribe('hashblock', emitter3);
      iopd.subscriptions.hashblock.length.should.equal(1);
      iopd.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      iopd.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(iopd.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var iopd = new BitcoinService(baseConfig);
      var emitter = new EventEmitter();
      iopd.subscribeAddress(emitter, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will handle multiple address subscribers', function() {
      var iopd = new BitcoinService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      iopd.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      iopd.subscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      iopd.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
      iopd.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      iopd.unsubscribeAddress(emitter1);
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      iopd.unsubscribeAddress(emitter1, ['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs']);
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter2];
      iopd.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      iopd.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      iopd.unsubscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.not.exist(iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      iopd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      sinon.spy(iopd, 'unsubscribeAddressAll');
      iopd.unsubscribeAddress(emitter1);
      iopd.unsubscribeAddressAll.callCount.should.equal(1);
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      iopd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      iopd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      iopd.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      iopd.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'] = [emitter1];
      iopd.unsubscribeAddress(emitter1);
      iopd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      iopd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
      iopd.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(iopd.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var iopd = new BitcoinService(baseConfig);
      var config = iopd._getDefaultConfig();
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a bitcoin.conf file', function() {
      var TestBitcoin = proxyquire('../../lib/services/iopd', {
        fs: {
          readFileSync: readFileSync,
      ...
      var iopd = new TestBitcoin(baseConfig);
      iopd.options.spawn.datadir = '/tmp/.bitcoin';
      var node = {};
      iopd._loadSpawnConfiguration(node);
      should.exist(iopd.spawn.config);
      iopd.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var iopd = new TestBitcoin(config);
      iopd.options.spawn.datadir = './data';
      var node = {};
      iopd._loadSpawnConfiguration(node);
      iopd.options.spawn.datadir.should.equal('/tmp/.bitcore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      ...
      var iopd = new TestBitcoin(baseConfig);
      (function() {
        iopd._loadSpawnConfiguration({datadir: './test'});
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the bitcoin.conf file', function() {
      ...
      var node = {};
      iopd._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        iopd.transactionDetailedCache.set(keys[i], {});
        iopd.utxosCache.set(keys[i], {});
        iopd.txidsCache.set(keys[i], {});
        iopd.balanceCache.set(keys[i], {});
        iopd.summaryCache.set(keys[i], {});
      }
      iopd._resetCaches();
      should.equal(iopd.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(iopd.utxosCache.get(keys[0]), undefined);
      should.equal(iopd.txidsCache.get(keys[0]), undefined);
      should.equal(iopd.balanceCache.get(keys[0]), undefined);
      should.equal(iopd.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      ...
      iopd._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        iopd.nodes[0].client.getInfo.callCount.should.equal(1);
        iopd.nodes[1].client.getInfo.callCount.should.equal(1);
        iopd.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
    it('will start using the current node index (round-robin)', function(done) {
      ...
      iopd._tryAllClients(function(client, next) {
        ...
        iopd.nodes[1].client.getInfo.callCount.should.equal(1);
        iopd.nodes[2].client.getInfo.callCount.should.equal(1);
        iopd.nodesIndex.should.equal(2);
        done();
      });
    });
  ...
  describe('#_wrapRPCError', function() {
    it('will convert iopd-rpc-dash error object into JavaScript error', function() {
      var iopd = new BitcoinService(baseConfig);
      var error = iopd._wrapRPCError({message: 'Test error', code: -1});
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      iopd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      iopd.nodes.push({
        client: {
      ...
      iopd._initChain(function() {
        log.info.callCount.should.equal(1);
        iopd.getRawBlock.callCount.should.equal(1);
        iopd.getRawBlock.args[0][0].should.equal('genesishash');
        iopd.height.should.equal(5000);
        iopd.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      iopd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      iopd._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    ...
    it('will get default rpc port for livenet', function() {
      ...
      var iopd = new BitcoinService(config);
      iopd._getDefaultConf().rpcport.should.equal(9998);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var iopd = new BitcoinService(config);
      iopd._getDefaultConf().rpcport.should.equal(19998);
    });
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var iopd = new BitcoinService(config);
      should.equal(iopd._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var iopd = new BitcoinService(config);
      iopd._getNetworkConfigPath().should.equal('testnet3/dash.conf');
    });
    it('will get default rpc port for regtest', function() {
      bitcore.Networks.enableRegtest();
      var config = {
      ...
      var iopd = new BitcoinService(config);
      iopd._getNetworkConfigPath().should.equal('regtest/dash.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var iopd = new BitcoinService(baseConfig);
      iopd.node.network = bitcore.Networks.testnet;
      iopd._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var iopd = new BitcoinService(baseConfig);
      iopd.node.network = bitcore.Networks.testnet;
      bitcore.Networks.enableRegtest();
      iopd._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var iopd = new BitcoinService(baseConfig);
      iopd.node.network = bitcore.Networks.livenet;
      bitcore.Networks.enableRegtest();
      should.equal(iopd._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      iopd._rapidProtectedUpdateTip = sinon.stub();
      iopd.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      iopd._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      iopd._rapidProtectedUpdateTip = sinon.stub();
      iopd._zmqBlockHandler(node, message);
      iopd._rapidProtectedUpdateTip.callCount.should.equal(1);
      iopd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      iopd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      iopd._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      iopd.subscriptions.hashblock.push(emitter);
      emitter.on('iopd/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        iopd._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.syncPercentage = sinon.stub();
      iopd.on('error', function(err) {
      ...
      iopd._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      iopd.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      iopd.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      iopd.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.syncPercentage = sinon.stub();
      iopd._resetCaches = sinon.stub();
      iopd.on('tip', function(height) {
        iopd._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        iopd.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      iopd.on('tip', function() {
        iopd.syncPercentage.callCount.should.equal(0);
        done();
      });
      iopd._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using bitcore.Transaction', function() {
      ...
      tx.sign(privkey);
      var addresses = iopd._getAddressesFromTransaction(tx);
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      iopd._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      iopd.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('iopd/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      iopd._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      iopd.subscriptions.rawtransaction.push(emitter);
      emitter.on('iopd/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      iopd._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      iopd.subscriptions.rawtransaction.push(emitter);
      emitter.on('iopd/rawtransaction', function() {
        done();
      });
      var node = {};
      iopd._zmqTransactionHandler(node, expectedBuffer);
  ...
  describe('#_zmqTransactionLockHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      iopd.subscriptions.transactionlock.push(emitter);
      emitter.on('iopd/transactionlock', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      iopd._zmqTransactionLockHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      var iopd = new BitcoinService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      iopd.subscriptions.transactionlock.push(emitter);
      emitter.on('iopd/transactionlock', function() {
        done();
      });
      var node = {};
      iopd._zmqTransactionLockHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var iopd = new BitcoinService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      iopd.on('txlock', function(buffer) {
        buffer.should.be.instanceof(Buffer);
        buffer.toString('hex').should.equal(expectedBuffer.toString('hex'));
        done();
      });
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd._updateTip = sinon.stub();
      iopd._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      iopd.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      iopd._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        iopd._updateTip.callCount.should.equal(11);
        iopd._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        iopd._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      iopd._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        iopd._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      iopd._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        iopd._zmqBlockHandler.callCount.should.equal(0);
        iopd._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var iopd = new BitcoinService(baseConfig);
      var node = {};
      iopd._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      iopd._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      iopd._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      iopd._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestBitcoinService = proxyquire('../../lib/services/iopd', {
        fs: {
          readFile: readFile
        }
      });
      var iopd = new TestBitcoinService(baseConfig);
      iopd.spawnStopTime = 1;
      iopd._process = {};
      iopd._process.kill = sinon.stub();
      iopd._stopSpawnedBitcoin(function(err) {
        if (err) {
          return done(err);
        }
        iopd._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      iopd._process.kill = sinon.stub().throws(error2);
      iopd._stopSpawnedBitcoin(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd._loadSpawnConfiguration = sinon.stub();
      iopd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      iopd._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give error from stopSpawnedBitcoin', function() {
      var iopd = new BitcoinService(baseConfig);
      iopd._loadSpawnConfiguration = sinon.stub();
      iopd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
      iopd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var iopd = new TestBitcoinService(config);
      iopd.spawn = {};
      iopd._loadSpawnConfiguration = sinon.stub();
      iopd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
      iopd.node.stopping = true;
      iopd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      iopd._loadSpawnConfiguration = sinon.stub();
      iopd.spawn = {};
      iopd.spawn.exec = 'testexec';
      iopd.spawn.configPath = 'testdir/bitcoin.conf';
      iopd.spawn.datadir = 'testdir';
      iopd.spawn.config = {};
      iopd.spawn.config.rpcport = 20001;
      iopd.spawn.config.rpcuser = 'bitcoin';
      iopd.spawn.config.rpcpassword = 'password';
      iopd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      iopd.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';

      iopd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      iopd._initZmqSubSocket = sinon.stub();
      iopd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      iopd._checkReindex = sinon.stub().callsArgWith(1, null);
      iopd._spawnChildProcess(function(err, node) {
        ...
        iopd._loadTipFromNode.callCount.should.equal(1);
        iopd._initZmqSubSocket.callCount.should.equal(1);
        should.exist(iopd._initZmqSubSocket.args[0][0].client);
        iopd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        iopd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(iopd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn iopd spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestBitcoinService = proxyquire('../../lib/services/iopd', {
        fs: {
      ...
      iopd._loadSpawnConfiguration = sinon.stub();
      iopd.spawn = {};
      iopd.spawn.exec = 'iopd';
      iopd.spawn.datadir = '/tmp/bitcoin';
      iopd.spawn.configPath = '/tmp/bitcoin/bitcoin.conf';
      iopd.spawn.config = {};
      iopd.spawnRestartTime = 1;
      iopd._loadTipFromNode = sinon.stub().callsArg(1);
      iopd._initZmqSubSocket = sinon.stub();
      iopd._checkReindex = sinon.stub().callsArg(1);
      iopd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      iopd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
      sinon.spy(iopd, '_spawnChildProcess');
      iopd._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            iopd._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      iopd._spawnChildProcess(function(err) {
        ...
        iopd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        iopd.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn iopd spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestBitcoinService = proxyquire('../../lib/services/iopd', {
        fs: {
      ...
      iopd._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            iopd._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      iopd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      iopd.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';
      iopd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      iopd._spawnChildProcess(function(err) {
        iopd._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      iopd._initZmqSubSocket = sinon.stub();
      iopd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      iopd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var iopd = new BitcoinService(config);
      iopd.node.stopping = true;
      iopd.startRetryInterval = 100;
      iopd._loadTipFromNode = sinon.stub();
      iopd._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        iopd._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      iopd.startRetryInterval = 1;
      var config = {};
      iopd._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        iopd._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      iopd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      iopd._connectProcess(config, function(err, node) {
        should.not.exist(err);
        iopd._loadTipFromNode.callCount.should.equal(1);
        iopd._initZmqSubSocket.callCount.should.equal(1);
        iopd._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error from connectProcess', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      iopd.options = {
        connect: [
          {}
        ]
      ...
      iopd.start(function(err) {
        iopd._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var iopd = new BitcoinService(baseConfig);
      var node = {};
      iopd._initChain = sinon.stub().callsArg(0);
      iopd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      iopd.options = {
        spawn: {}
      };
      iopd.start(function(err) {
        should.not.exist(err);
        iopd.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      iopd._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      iopd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      iopd.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      iopd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      iopd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      iopd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      iopd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      iopd.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      iopd.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var iopd = new BitcoinService(baseConfig);
      var args = iopd._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var iopd = new BitcoinService(baseConfig);
      var args = iopd._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      iopd.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      iopd.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        iopd.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      iopd.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      iopd.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = iopd._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = iopd._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = iopd._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      iopd._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        iopd._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      iopd.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      iopd.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      iopd.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      iopd.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        iopd.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var iopd = new BitcoinService(baseConfig);
      iopd.height = 10;
      var confirmations = iopd._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var iopd = new BitcoinService(baseConfig);
      var result = {};
      iopd._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var iopd = new BitcoinService(baseConfig);
      var result = {};
      iopd._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var iopd = new BitcoinService(baseConfig);
      var result = {};
      iopd._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var iopd = new BitcoinService(baseConfig);
      var result = {};
      iopd._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = iopd._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var iopd = new BitcoinService(baseConfig);
      iopd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      iopd.height = 300;
      var addresses = {};
      iopd._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      iopd._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var iopd = new BitcoinService(baseConfig);
      iopd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      iopd._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from bitcore addresses', function() {
      ...
      var iopd = new BitcoinService(baseConfig);
      var strings = iopd._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var iopd = new BitcoinService(baseConfig);
      (function() {
        iopd._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      iopd.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      iopd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      iopd.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var iopd = new BitcoinService(baseConfig);
      iopd.maxAddressesQuery = 100;
      iopd.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      iopd.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      iopd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      iopd.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      iopd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      iopd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      iopd.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      iopd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      iopd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      iopd.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(iopd, '_paginateTxids');
      iopd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      iopd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      iopd.getAddressSummary(address, options, function(err, summary) {
        iopd._paginateTxids.callCount.should.equal(1);
        iopd._paginateTxids.args[0][1].should.equal(0);
        iopd._paginateTxids.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      iopd.getAddressSummary(address, options, function(err, summary) {
        ...
        iopd.getAddressTxids.callCount.should.equal(1);
        iopd.getAddressBalance.callCount.should.equal(1);
        iopd.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          iopd.getAddressTxids.callCount.should.equal(1);
          iopd.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      iopd.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginateTxids', function(done) {
      ...
      iopd._paginateTxids = sinon.stub().throws(new Error('test'));
      var address = '7oK6xjGeVK5YCT5dpqzNXGUag1bQadPAyT';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      iopd.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      iopd.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      iopd.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      iopd.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will getblock as bitcore object from height', function(done) {
      ...
      iopd.getBlock(0, function(err, block) {
        should.not.exist(err);
    ...
    it('will getblock as bitcore object', function(done) {
      ...
      iopd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
    ...
    it('will get block from cache', function(done) {
      ...
      iopd.getBlock(hash, function(err, block) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        getBlock.callCount.should.equal(1);
        block.should.be.instanceof(bitcore.Block);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      iopd.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      iopd.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      iopd.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      iopd.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      iopd.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      iopd.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      iopd.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      iopd._maybeGetBlockHash('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      iopd._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      iopd._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      iopd.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      iopd.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      iopd.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      iopd.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    ...
    it('will give rpc error', function() {
      ...
      iopd.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      iopd.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      iopd.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      (function() {
        iopd.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      iopd.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      iopd.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      iopd.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      iopd.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      iopd.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      iopd.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      iopd.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      iopd.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      iopd.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      iopd.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    ...
    it('will call client getInfo and give result', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.node.getNetworkName = sinon.stub().returns('testnet');
      ...
      iopd.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
  ...
  describe('#govObject', function() {
    it('will call client gobject list and give result', function(done) {
        ...
        iopd.govObjectList({type: 1}, function(err, result) {
            if (err) {
                return done(err);
            }
            should.exist(result);
    ...
    it('will call client gobject get and give result', function(done) {
      ...
      iopd.govObjectHash('4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00', function(err, result) {
        if (err) {
          return done(err);
        }
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      iopd.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      iopd.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.spawn = {};
      iopd.spawn.process = new EventEmitter();
      iopd.spawn.process.kill = sinon.stub();
      iopd.stop(done);
      iopd.spawn.process.kill.callCount.should.equal(1);
      iopd.spawn.process.kill.args[0][0].should.equal('SIGINT');
      iopd.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      iopd.spawn.process.kill.callCount.should.equal(1);
      iopd.spawn.process.kill.args[0][0].should.equal('SIGINT');
      iopd.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var iopd = new BitcoinService(baseConfig);
      iopd.shutdownTimeout = 300;
      iopd.spawn = {};
      iopd.spawn.process = new EventEmitter();
      iopd.spawn.process.kill = sinon.stub();
      iopd.stop(function(err) {
